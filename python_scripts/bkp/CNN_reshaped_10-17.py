...
for set_i in range(start_set, end_set):
    ...
    training_size=sum(receptor_sizes[start_set:end_set])-test_size

    l_count=0
    training_set=np.zeros((training_size, size*size*3+300))
    training_labels=np.zeros((training_size, 2))
    
    for set_j in range(start_set, end_set):
        if set_j != set_i:
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True,log_device_placement=True)) as sess:
        ...
        total_batch_test = int(test_size / batch_size_test)
...
#                _, c = sess.run([optimiser, cross_entropy], feed_dict={x: batch_x, y: batch_y, z: batch_z})
