...
receptors = sorted(glob.glob(directory + 'data/pockets_dude_mol2_updated/*.mol2'))
input_folder=directory + 'data/pockets_dude_mol2_updated'
savepath=directory+'train_test_data/'+date_str
dpi=20
size=32
receptors_folder=directory + 'data/pockets_dude_tiff_'+str(size)+'/'
ligands_folder = glob.glob(directory + 'data/screen-libs-sdf/*.sdf')
...
#    from mol2vec.helpers import depict_identifier, plot_2D_vectors, IdentifierTable, mol_to_svg
    ...
    #    if receptor_name+'_'+str(label) not in d_smiles.keys():
    #        d_smiles[receptor_name+'_'+str(label)]=[]
    ...
    #        smiles=dicts['SMILES_CANONICAL']
    #        if smiles not in smiles_list:
    #            smiles_list.append(smiles)
    #        
    #        d_smiles[receptor_name+'_'+str(label)].append(smiles)
    #    print(str(r_num), " th receptor-label + ", str(l_num) , 'th ligand')
        print(str(l_num), " th receptor")
        l_num = l_num+1
    #    r_num=r_num+1
     
    #save_obj(d_smiles, directory + 'train_test_data/'+date_str+'/receptorname_ligand_dict_smiles'+str(date_str))
...
if generate_images:
    ...
    for filename in receptors:
        receptor=filename.split('/')[-1].split('_')[0]
        image_filename=receptors_folder+receptor+'.tiff'
        try:
            deepdrugV.voronoi_atoms(filename, image_filename, dpi, size)
...
#                print(np.sum(test_labels[:,0]),'   ',np.sum(test_labels[:,1]))
                ...
                print("Epoch:", (epoch + 1), "batch:", (i + 1), "cost =", "{:.3f}".format(avg_cost), " test accuracy: {:.3f}".format(test_acc))
                print("Epoch:", (epoch + 1), "batch:", (i + 1), "confusion matrix: ",sess.run(confusion, feed_dict={x: test_set[:,300:], y: test_labels,  z: test_set[:,0:300]}))
